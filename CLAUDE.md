...
3. **コントローラ層 (`assets/app.js`)**:
   ...
   - 順位推移グラフサブタブ (`subpanel-chart`): `renderRankingChart()` がピュア SVG (折れ線 + 順位ラベル) を生成し、チェックボックスのトグルで `refreshChart()` が再描画。CDN ライブラリ不使用。
     ...
     - **類似軌跡検索**: 各ピッカー行の「類似」ボタン → `openSimilarityModal(seasonId, imageId)` がモーダル表示。**Landmark-based piecewise-linear warping** で時系列をアライメント:
       ...
       - `buildAlignedTrajectory(trainee, season, warping)` が warped x × `(rank-1)/(total-1)` の点列を返す。各点は `status: 'observed' | 'final_pad'` を持つ: `rank_history[key] == null` (= 脱落確定) の milestone は **最終順位** (`trainee.rank` → 最後の observed rank → `total_trainees` の優先順) で水平 padding され、`final_pad` 印が付く。`rank_history` に key 自体が無い (進行中シーズンの未放送回など) milestone はスキップ。`resampleTrajectory()` で `N = max(32, 共通アンカー数 * 8)` 点に線形補間 → `trajectoryDistance()` が **位置 MAE (重み 0.7) + 傾き MAE (`|Δa[i] - Δb[i]|` 平均, 重み 0.3) + overlap penalty (0.02 × max(0, 6 - count))** を返す。傾き項により「上昇 vs 下降」「同形・異ベースライン」が明示的に区別される。距離計算は常に [0,1] グリッドで行うので、x>1 への外挿区間はチャート描画にのみ寄与する。`similarTrainees()` の距離同値タイブレークは observed 点数の多い候補を優先 (= padding 比率の低い、より確かな比較を上に)。
