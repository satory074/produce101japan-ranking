...
| ベネフィット (得票加算) を追加・修正 | `data/<season>.json` | 該当 trainee に `benefit_history: { <milestone.key>: <票数> }` を追記。§ベネフィット運用 参照 |
...
### ベネフィット (`benefit_history`)

任意フィールド。各 trainee に `benefit_history: { <milestone.key>: <票数(整数)> }` を持たせ、ミッション (グループバトル / ポジション評価 / コンセプト評価) で獲得した得票加算ボーナスを記録する。

- 値 = 票数 (整数)。**0 のときはキーごと省略** (= 「未獲得」)。
- ミッション系 milestone (`p2`, `p3`, `p4`, ...) に紐付けるのが原則。THE GIRLS のように「コンセプト評価」専用 milestone key が無いシーズンでは、**直後の ceremony key (例: `rc3`) を例外的に使う**。
- 同一 milestone 内に複数のベネフィット (例: SEASON 1 グループバトルの「グループ内1位 +20,000」と「全体MVP +50,000」) が同一人物に重なる場合は **合算** して 1 つの値にする。`assets/app.js` の取り込みスクリプトでは前回値に加算する実装。
- 表示への影響:
  - **順位推移表**: ベネフィットを持つセルは順位バッジ右に `+10万` / `+3.1k` などの圧縮表示が併記される (`formatBenefitShort()`)。`title` 属性に正確な値、ヘッダには ★ バッジ。
  - **順位推移グラフ**: 該当 milestone 点の右下に金色 ★ マーカー。tooltip にミッション別 + 累計票数行が出る。
  - **類似軌跡オーバーレイ**: 基準軌跡 (黒太線) にのみ ★ を描画 (候補側は省略、混雑防止)。

データ取り込み方針:
- SHINSEKAI グループバトル全 8 戦 / SEASON 1〜2 グループバトル + ポジション + コンセプト / THE GIRLS ポジション + コンセプトをカバー済み。
- 公開情報が乏しい区間 (例: SEASON 2 ポジション、THE GIRLS グループバトルの全 8 曲分メンバー) は **キー省略** で「表示しない」。後日追加可能な拡張性は確保。
- 主な情報源: `kpop-oyaji.com` (SHINSEKAI), `imokorori.com` (SEASON 1/2), `encount.press` (THE GIRLS コンセプト), `magazine.fany.lol` (各種), Wikipedia。

## デビュー組の判定

`debuted: true` のカードは黄色枠 + DEBUT バッジ + 順位バッジが金色 (rank=1)・銀色 (2)・銅色 (3)・通常黄 (4 〜 `debutCap`) になる。「デビュー組のみ」絞り込みは `data-rank <= debutCap` で判定するため、SHINSEKAI のように `debuted` が全員 false の放送中シーズンでも Top `debutCap` の暫定枠は表示される。
