...
- 「全シーズン / このシーズン以外」トグル付き。**既定値は「このシーズン以外」** (異シーズン比較が機能の主目的のため、commit `1c41176`)。`openSimilarityModal` の `root.dataset.filter === 'all' ? 'all' : 'other'` フォールバックで実装。close 時に `dataset.filter` をクリアするため毎回既定にリセットされる。結果行のアクションは常に「`{シーズン短縮名}` を開く」ボタンで、同・別シーズン問わず `renderSimilarityModal` を新基準で再描画 (= モーダル内で基準切替)。
...
- **SHINSEKAI**: `{ rank, name_jp, name_romaji, stage_name, image_id, debuted, ongoing_rank, global_ranker? }` — **完結 (FINAL 2026-06-06)**。`votes_final` / `eliminated_at` なし、代わりに `stage_name` (カード下部に小さく表示) と `ongoing_rank` (放送中のファイナル進出フラグ、現在は履歴的・app.js 未使用) を持つ。RC3 で Top 20 main + Global Ranker 2 名 = 22 人がファイナル進出し、FINAL で 1〜22 位に再採番された (Top 12 = `debuted: true`)。`ongoing_rank` / `global_ranker` は app.js では参照されないデータ専用フィールド。
  - **`rank` 復元ポリシー** (commits `106e8a3` / `112fe55`): 脱落者は **最終観測順位を `rank` に保持** する規約 — RC3 脱落者は `rank = rank_history.rc3`、RC2 脱落者は `rank = rank_history.rc2`、RC1 脱落者は `rank = rank_history.rc1`。これによりカードバッジに数字 (濃グレー帯) が出続け、`-` 表示にはならない。
  - **`rank: null` は辞退者のみ**: 唯一の例外は KINARI (`kenmotsukinari`)。コンセプト評価後 (2026-05-25) に辞退、`rc2=4` 時点で離脱したため最終観測順位を残すと「4位」と誤解させるので意図的に `rank: null` に。トップレベル `notes` に辞退の事実を追記する規約 (専用フィールドなし)。**新たな辞退者が出た場合は、その時点で上位にいたかどうかで `rank` を null にするか rc-N にするか判断する**。
...
5. `position_battle`: `{ song, team, result }` — ポジションバトル。**S1 のみ Ep 3-4 (= group_battle の前)、S2 / TG / SHINSEKAI は Ep 6-7 前後** に放送。
   ...
   - **THE GIRLS**: 10 曲単独チーム (V=4/D=4/**Rap&Vocal=2**)。番組はテーマ別チーム名 (ヒーローズ / Shines / peony / Track Beat / ウチら 等) と V/D/R/Rap&Vocal の併記が標準。team は `"{theme} ({V/D/R | Rap&Vocal})"` 形式 (例: `"ヒーローズ (Vocal)"`, `"peony (Dance)"`, `"ウチら (Rap&Vocal)"`, `"Track Beat (Rap&Vocal)"`)。`result: "win"` のみ勝者。出典: Wikipedia
...
- SHINSEKAI (6 points, 完結): `https://produce101.jp/profile/data/?id={image_id}` → `p1 / p2 / rc1 / rc2 / rc3 / rcF` (rcF = FINAL 2026-06-06。非ファイナリストは `NULL`)
...
`debuted: true` のカードは黄色枠 + DEBUT バッジ + 順位バッジが黄色 (Top `debutCap` まで一律 `bg-yellow-400 text-yellow-900`) になる。`debuted` が全員 false の放送中シーズン (= 過去の SHINSEKAI RC3 時点) でも Top `debutCap` の暫定枠は同様にハイライトされる (全 4 シーズン完結済みの現在は実デビュー組がハイライトされる)。
...
**完結済み (FINAL 2026-06-06、デビュー組 KO1KEYZ)**。`ongoing: false`、`rank`/`rank_history.rcF` は最終順位 1〜22、Top 12 が `debuted: true`。以下は放送中だった当時の更新手順 (アーカイブ目的で保持。完結後の唯一の追記候補は公式が最終票数を公表した場合の `votes_final` と、FINAL 課題曲の `debut_evaluation`)。

放送中は毎週更新が必要だった。`data/shinsekai.json` のみ書き換えれば良い:
- `last_updated_episode` を更新 (例: `"Episode 9 (2026-05-15)"`)
- `trainees[].rank` を最新の順位発表式の結果で並び替え
- 新規脱落した練習生は `ongoing_rank: false` にし、`rank` は **その回の `rank_history` 値を残す** (= 最終観測順位を保持する規約、§データスキーマの注意点 参照)。`rank: null` にするのは辞退者のみ (KINARI のような例外ケース)
...
最終回後 (実施済み): `ongoing: false` に変更し、22 人のファイナリストに `rank_history.rcF` (最終順位) を追記して `rank` を同期、`debuted: true` を Top `debut_count` (12) にセット、`debut_group: "KO1KEYZ"` を記入。**順位重複の解消**: ファイナリストが 1〜22 を占めるため、RC3 脱落で `rank=22` だった tanakamaki を 23 に繰り上げ (小清水蓮が Global Ranker で勝ち上がり最終 20 位 → 23 位が空く)。完結済み 3 シーズン共通の「順位重複なし」不変条件を維持するための判断 (commit はこの更新)。`votes_final` はランクグラフ API が票数を返さないため未取り込み (公式が公表すれば後日追加)。
